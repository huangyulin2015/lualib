...
local function sqliteEscape(keyWord)
...
--    keyWord = string.gsub(keyWord, "%%", "/%%")
--    keyWord = string.gsub(keyWord, "&", "/&")
--    keyWord = string.gsub(keyWord, "_", "/_")
--    keyWord = string.gsub(keyWord, "%(", "/(")
--    keyWord = string.gsub(keyWord, "%)", "/)")
    return keyWord
end
...
function Compile.CREATE(Object)
    local tCreate = Object._create
    --    local sReturn = ("INSERT INTO `%s` ( %s )\nVALUES\n(\n\t%s\n)"):format(tInsert.tbl, table.concat(tInsert.cols, ', '), table.concat(tInsert.val, ',\n\t'))

    local sReturn = ("CREATE TABLE `%s` (\n %s \n);"):format(tCreate.tbl, tCreate.rows)
    return sReturn
end
...
function luaqub:where(clauses, value, joiner)
    ...
    if type(clauses) == "string" then
        ...
        table.insert(self._where, { join = joiner:upper(), statement = Trim(clauses) .. " '" .. sqliteEscape(Trim(value)) .. "'" })
        clauses = nil
    elseif type(clauses) == "table" then
        ...
        for Key, Value in pairs(clauses) do
            if tonumber(Key) then
                ...
                table.insert(self._where, { join = joiner, statement = ("%s = %s"):format(Trim(Key), "'" .. sqliteEscape(Trim(Value)) .. "'") })
            end
        end
        clauses = nil
    end
...
function luaqub:insert(tbl, datas)
    ...
    for Key, Value in pairs(datas) do
        ...
        if not tonumber(Value) then
            table.insert(tValue, ("'%s'"):format(sqliteEscape(Trim(Value))))
        else
...
function luaqub:create(tbl, datas)
    if not datas or type(datas) ~= 'table' then
        error("Unexpected argument #2 passed to insert function.")
        return false
    end
    ...
    for key, value in pairs(datas) do
        ...
        table.insert(insertArr, str .. typeStr)
    end
    self._flag, self._create.rows, self._create.tbl = 'create', _.join(insertArr, ",\n"), tbl
    return self
end
